...
import matplotlib.gridspec as gridspec  # grid view
from Plate_detect_and_recognize.local_utils import detect_lp  # plate detection from wpod pretrained model
from os.path import splitext  # part of text
from keras.models import model_from_json  # loading the trained model from json file
from sklearn.preprocessing import LabelEncoder  # from o to classes-1
...
def show_upload_button(welcome_btn):
    ...
    label.pack()
    label.place(x=150, y=600)


# browse image-displaying file dialog
def upload_image():
    try:
...
# displaying button to detect license plate and recognise the characters
...
# detection of license plate from vehicle image
def get_plate(image_path, Dmax=608, Dmin=265):
    vehicle = preprocess_image(image_path)
    ratio = float(max(vehicle.shape[:2])) / min(vehicle.shape[:2])  # height/width ratio of the image
    side = int(ratio * Dmin)
    bound_dim = min(side, Dmax)
    _, LpImg, _, cor = detect_lp(wpod_net, vehicle, bound_dim,
                                 lp_threshold=0.5)  # lpthreshold taken as0 if less or 1 if greater
    return vehicle, LpImg, cor


# pre-process image using computer vision or opencv
def preprocess_image(image_path, resize=False): # resizing the image
    img = cv2.imread(image_path) # loading the image
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    img = img / 255  #to convert into binary(255)
    if resize:
        img = cv2.resize(img, (224, 224))
    return img


# sort contoursbased on x-coordinate so that the characters are in order
def sort_contours(cnts, reverse=False):
    i = 0
    boundingBoxes = [cv2.boundingRect(c) for c in cnts]
    # ascending order of x-coordinates to get everything in order
    (cnts, boundingBoxes) = zip(*sorted(zip(cnts, boundingBoxes),key=lambda b: b[1][i], reverse=reverse))
    return cnts


# pre-process each character image and predict with output label using trained model
...
# output- license plate detection,recognition and e-mail
def detect_and_recognize(file_path):
    ...
    for i in range(len(plot_image)):
        ...
        if i == 0:
            plt.imshow(plot_image[i])  #only first image needs to be in original format
        else:
            plt.imshow(plot_image[i], cmap="gray")  #rest all images are to be printed in gray
    # plt.show()
    ...
    for c in sort_contours(cont):
        ...
        if 0.5 <= ratio <= 4:  # Only select contour with defined ratio
            if h / plate_image.shape[0] >= 0.4:  # 0.4 Select contour which has the height larger than 40% of the plate
    ...
    # plt.show()
    # all the characters are segmented and the image is saved
    ...
    final_string = ''
    # enumerate means it returns a list with its character and its index
    ...
    plate_image.pack()  # loading the final image
...
# loading wpodnet pre-trained model
def load_model(path):
    try:
...
if __name__ == '__main__':
    ...
    ws = gui.winfo_screenwidth()  # width of the desktop screen
    hs = gui.winfo_screenheight()  # height of the desktop  screen
    ...
    gui.iconphoto(True, PhotoImage(file="logo.png"))
    loc_icon = Image.open("logo.png")  # display icon on window
    ...
    # label for displaying the following text
